...
__global__ void single_mem_bw (float* A,  float* B, float* C){
	// block and thread index
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(){
	...
	// printf("float\n");
	// single_mem_bw<<<BlockNums, THREADS_NUM>>>(A_g, B_g, C_g);
	...
	for (int i = BENCH_ITER - 1; i >= 0; --i) {
		...
		// single_mem_bw<<<BlockNums, THREADS_NUM>>>(A_g + i * MEMORY_OFFSET, B_g + i * MEMORY_OFFSET, C_g + i * MEMORY_OFFSET);
	}
